local PANEL = {}
    ...
    function PANEL:Init()
		...
		if (IsValid(nut.gui.skills)) then
			nut.gui.skills:Remove()
		end
		...
		for k, v in SortedPairsByMemberValue(nut.skillCategories.list, "order") do
		...
		for k, v in pairs( nut.skills.list) do
			if IsValid(self.panels[v.category]) then
				...
				self.panels[v.category].skills = {}

				local skill = self:AddSkill(v, k, (self.panels[v.category].x), self.panels[v.category].y, self.panels[v.category])
				self.panels[v.category].skills[k] = skill
			end
	...
	function PANEL:AddSkill(skill, key, x, y, panel)
...
vgui.Register("skillsInfoStep", PANEL, "DFrame")

-------------------------------------------------------------------------------------------------------------------------------------------------

-------------------------------------------------------------------------------------------------------------------------------------------------

net.Receive( "openSkills", function( len )
