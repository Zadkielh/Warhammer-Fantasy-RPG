...
hook.Add( "PostDrawTranslucentRenderables", "Celestial_Lightning", function()
	--if ( #segments < 1 || n < CurTime() ) then
		...
		--n = CurTime() + .01
	--end
	local MaterialMain			= Material( "hd/effects/lightning/lightning_arc_sprite" );
	local MaterialFront			= Material( "hd/effects/lightning/lightning_arc_sprite" );
...
function EFFECT:Init( data )
	...
	print("test")
	if (IsValid(ent)) then
		GenerateLighting( pos, ent, math.random( 10, 20 ), 5 )
	else
		GenerateLighting( startpos, pos, math.random( 10, 20 ), 5 )
	end
