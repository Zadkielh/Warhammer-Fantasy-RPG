...
*/
local mats = {
	(Material( "hd/effects/lightning/lightning_purple" )),
	--(Material( "hd/effects/lightning/lightning_green" ))
...
hook.Add( "PostDrawTranslucentRenderables", "Dark_Lightning", function()
	--if ( #segments < 1 || n < CurTime() ) then
		...
		--n = CurTime() + .01
	--end
	local MaterialMain			= Material( "hd/effects/lightning/dark_lightning_main" );
...
function EFFECT:Init( data )
	...
	print("test")
	if (IsValid(ent)) then
		GenerateLighting( pos, ent, math.random( 20, 30 ), 5 )
	else
		GenerateLighting( startpos, pos, math.random( 20, 30 ), 5 )
	end
	

	table.insert( segments, {
		segs = GenerateLighting( startpos, pos, math.random( 10, 20 ), 4 ),
