...
function ENT:CustomOnInitialize()
	...
	ParticleEffectAttach("fantasy_fireball_staff", PATTACH_ABSORIGIN_FOLLOW, self, 0)
...
function ENT:CustomOnThink()
	...
	if self.velocity <= 1000 then
		...
		self.velocityUp = self.velocityUp * 1
	end

	local phys = self:GetPhysicsObject()
    if IsValid(phys) then -- If the physics object is valid
        phys:SetVelocity(self:GetForward() * self.velocity + self:GetUp()*self.velocityUp)
	end
end
