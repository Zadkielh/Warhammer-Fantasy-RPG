...
nut.chat.register("howl", {
    onCanSay =  function(speaker, text)
        if !(speaker:getChar():hasFlags("GM")) then
            speaker:notifyLocalized("You are not a vortigaunt.")
    ...
    onChatAdd = function(speaker, text)
        if (LocalPlayer():getChar():hasFlags("GM")) then
            chat.AddText(Color(77, 158, 154), speaker:Nick().." howls: "..text)
